...
public class FollowBezierCurve : MonoBehaviour 
{
    ...
    public void Update()
    {
        _transform.position = bezier(Mathf.Clamp01((Time.time - _startTime) / duration), points);
    }
    ...
    public void OnDrawGizmos()
    {
        ...
        for (float i = 0f; i <= (float)GIZMO_CURVE_LINE_SEGMENTS; i += 1f) {
            Vector3 pNew = bezier(Mathf.Clamp01(i / (float)GIZMO_CURVE_LINE_SEGMENTS), points);
